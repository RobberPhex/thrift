...
if test "$PHP_THRIFT_PROTOCOL" != "no"; then
  ...
  if test "$PHP_THRIFT_FOUND_VERNUM" -ge "70000"; then
    PHP_SUBST(THRIFT_PROTOCOL_SHARED_LIBADD)
    PHP_NEW_EXTENSION(thrift_protocol, php_thrift_protocol.cpp php_thrift_protocol_binary.cpp php_thrift_protocol_compact.cpp, $ext_shared)
