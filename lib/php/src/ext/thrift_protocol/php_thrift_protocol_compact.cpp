...
enum TCompactType {
  ...
  TCOMPACT_I32        = 0x05,
  TCOMPACT_I64        = 0x06,
  TCOMPACT_DOUBLE     = 0x07,
  TCOMPACT_BINARY     = 0x08,
  TCOMPACT_LIST       = 0x09,
  TCOMPACT_SET        = 0x0A,
  TCOMPACT_MAP        = 0x0B,
  TCOMPACT_STRUCT     = 0x0C
};
...
static
void compact_serialize_spec(zval* zthis, PHPOutputTransport& transport, HashTable* spec);
static
...
enum TCompactState {
  TCOMPACTSTATE_CLEAR           = 0,
  TCOMPACTSTATE_FIELD_WRITE     = 1,
  TCOMPACTSTATE_VALUE_WRITE     = 2,
  TCOMPACTSTATE_CONTAINER_WRITE = 3,
  TCOMPACTSTATE_BOOL_WRITE      = 4,
  TCOMPACTSTATE_FIELD_READ      = 5,
  TCOMPACTSTATE_CONTAINER_READ  = 6,
  TCOMPACTSTATE_VALUE_READ      = 7,
  TCOMPACTSTATE_BOOL_READ       = 8
};
public:
  TCompactProtocol(PHPOutputTransport* trans) {
    transport = trans;
  }
  void serialize(zval* zthis) {
    zval* _spec = zend_read_static_property(Z_OBJCE_P(zthis), "_TSPEC", sizeof("_TSPEC")-1, true);

    if (!_spec || Z_TYPE_P(_spec) != IS_ARRAY) {
    ...
    zval* val_ptr;
    HashTable* spec=Z_ARRVAL_P(_spec);
    ...
    for (zend_hash_internal_pointer_reset_ex(spec, &key_ptr);
      ...
      TType ttype = (TType)Z_LVAL_P(val_ptr);

      zval rv;
      zval* prop = zend_read_property(Z_OBJCE_P(zthis), zthis, varname, strlen(varname), false, &rv);
      if (Z_TYPE_P(prop) != IS_NULL) {
        this->writeField(varname, ttype, fieldno, prop, fieldspec);
      }
    }
    transport->writeI8(T_STOP);
  }
protected:
  TCompactType toTCompactType(TType ttype){
    switch(ttype){
    case TType::T_STOP:
      return TCompactType::TCOMPACT_STOP;
    case TType::T_BOOL:
      // used for collection
      return TCompactType::TCOMPACT_TRUE;
    case TType::T_BYTE:
      return TCompactType::TCOMPACT_BYTE;
    case TType::T_I16:
      return TCompactType::TCOMPACT_I16;
    case TType::T_I32:
      return TCompactType::TCOMPACT_I32;
    case TType::T_I64:
      return TCompactType::TCOMPACT_I64;
    case TType::T_DOUBLE:
      return TCompactType::TCOMPACT_DOUBLE;
    case TType::T_STRING:
      return TCompactType::TCOMPACT_BINARY;
    case TType::T_STRUCT:
      return TCompactType::TCOMPACT_STRUCT;
    case TType::T_LIST:
      return TCompactType::TCOMPACT_LIST;
    case TType::T_SET:
      return TCompactType::TCOMPACT_SET;
    case TType::T_MAP:
      return TCompactType::TCOMPACT_MAP;
    default:
      throw_tprotocolexception("Invalid TType", 0);
    }
  }
  size_t writeMessageBegin(zend_string *name, long msgtype, long seqID){
    size_t written =
      this->writeUByte(TCompactProtocol::PROTOCOL_ID) +
      this->writeUByte(TCompactProtocol::VERSION | (msgtype << TCompactProtocol::TYPE_SHIFT_AMOUNT)) +
      this->writeVarint(seqID) +
      this->writeString(ZSTR_VAL(name), ZSTR_LEN(name));

    return written;
  }
  size_t writeFieldValue(TType field_type, zend_ulong field_id, zval* value, HashTable* fieldspec) {
    if(field_type==TType::T_BOOL){
      if (!zval_is_bool(value)) convert_to_boolean(value);
      if(Z_TYPE_INFO_P(value) == IS_TRUE)
        return writeByte(TCompactType::TCOMPACT_TRUE);
      else
        return writeByte(TCompactType::TCOMPACT_FALSE);
    } else {
      return writeValue(field_type, value,  fieldspec);
    }
  }
  size_t writeValue(TType field_type, zval* value, HashTable* fieldspec) {
    switch(field_type){
    case TType::T_STOP:
      return 0;
    case TType::T_BOOL: {
      if (!zval_is_bool(value)) convert_to_boolean(value);
      return writeByte(Z_TYPE_INFO_P(value) == IS_TRUE ? 1 : 0);
    }
    case TType::T_BYTE: {
      if (Z_TYPE_P(value) != IS_LONG) convert_to_long(value);
      return writeByte(Z_LVAL_P(value));
    }
    case TType::T_I16: {
      if (Z_TYPE_P(value) != IS_LONG) convert_to_long(value);
      return writeI16(Z_LVAL_P(value));
    }
    case TType::T_I32: {
      if (Z_TYPE_P(value) != IS_LONG) convert_to_long(value);
      return writeI32(Z_LVAL_P(value));
    }
    case TType::T_I64: {
      int64_t l_data;
    ...
    case TType::T_STRING:{
      ...
      return writeString(Z_STRVAL_P(value), Z_STRLEN_P(value));
    }
    case TType::T_STRUCT:
      return TCompactType::TCOMPACT_STRUCT;
    case TType::T_LIST: {
      ...
      if (Z_TYPE_P(val_ptr) != IS_LONG) convert_to_long(val_ptr);
      TType valtype = (TType)Z_LVAL_P(val_ptr);
      val_ptr = zend_hash_str_find(fieldspec, "elem", sizeof("elem")-1);
      HashTable* valspec = Z_ARRVAL_P(val_ptr);

      if(size <= 14){
        written += writeUByte(size << 4 | toTCompactType(valtype));
      }else{
        written += writeUByte(0xf0 | toTCompactType(valtype));
        written += writeVarint(size);
      }
      ...
      for (zend_hash_internal_pointer_reset_ex(ht, &key_ptr);
        ...
        written += writeValue(valtype, val_ptr, valspec);
      }
      return written;
    }
    case TType::T_SET: {
      ...
      val_ptr = zend_hash_str_find(fieldspec, "etype", sizeof("etype")-1);
      if (Z_TYPE_P(val_ptr) != IS_LONG) convert_to_long(val_ptr);
      TType keytype = (TType)Z_LVAL_P(val_ptr);

      written += writeUByte(toTCompactType(keytype));
      written += writeI32(zend_hash_num_elements(ht));
      HashTable* valspec = Z_ARRVAL_P(zend_hash_str_find(fieldspec, "etype", sizeof("etype")-1));
      HashPosition key_ptr;
      for (zend_hash_internal_pointer_reset_ex(ht, &key_ptr);
           (val_ptr = zend_hash_get_current_data_ex(ht, &key_ptr)) != nullptr;
           zend_hash_move_forward_ex(ht, &key_ptr)) {
        written += writeValue(keytype, val_ptr, valspec);
      }
      return written;
    }
    case TType::T_MAP:
  ...
  size_t writeField(char *field_name, TType field_type, zend_ulong field_id,zval* value,HashTable* fieldspec) {
    size_t written = 0;
    written += writeFieldBegin(field_name, field_type, field_id);
    written += writeValue(field_type, value, fieldspec);
    return written;
  }
  size_t writeFieldBegin(char *field_name, TType field_type, zend_ulong field_id) {
    if(field_type==TType::T_BOOL)
      return 0;
    return this->writeFieldHeader(toTCompactType(field_type), field_id);
  }
  size_t writeFieldHeader(TCompactType type, zend_ulong field_id) {
    size_t written = 0;
    zend_ulong delta = field_id - this->lastFid;
    if (0 < delta && delta <= 15) {
      written = this->writeUByte((delta << 4) | type);
    } else {
      written = this->writeByte(type) +
        this->writeI16(field_id);
    }
    this->lastFid = field_id;

    return written;
  }
  size_t writeUByte(uint8_t byte) {
    return transport->write((const char*)&byte, 1);
  }
  size_t writeByte(int8_t byte) {
    return transport->write((const char*)&byte, 1);
  }
  size_t writeI16(int16_t i) {
    i = (i<<1) ^ (i>> 15);
    return writeVarint(i);
  }
  size_t writeI32(int32_t i) {
    i = (i<<1) ^ (i>> 31);
    return writeVarint(i);
  }
  size_t writeI64(int64_t i) {
    i = (i<<1) ^ (i>> 63);
    return writeVarint(i);
  }
  size_t writeString(char *value, size_t len) {
    size_t written = 0;
    written += writeVarint(len);
    written += transport->write(value, len);
    return written;
  }
  size_t writeVarint(int64_t data){
    size_t written = 0;
    while(true) {
      if((data & ~0x7f)==0) {
        written += transport->write((const char*)&data, 1);
        return written;
      } else {
        int8_t out = (data & 0xff) | 0x80;
        written += transport->write((const char*)&out, 1);
        data >>= 7;
      }
    }
  }
  PHPOutputTransport* transport;
  std::list<TCompactState> containers;
  zend_ulong lastFid = 0;
  int boolFid = 0;
...
static
void compact_serialize(int8_t thrift_typeID, PHPOutputTransport& transport, zval* value, HashTable* fieldspec) {
  ...
  switch (thrift_typeID) {
    ...
    case T_STRUCT: {
      ...
      compact_serialize_spec(value, transport, Z_ARRVAL_P(spec));
    } return;
    case T_BOOL:
      if (!zval_is_bool(value)) convert_to_boolean(value);
...
void compact_serialize_spec(zval* zthis, PHPOutputTransport& transport, HashTable* spec) {
  ...
  for (zend_hash_internal_pointer_reset_ex(spec, &key_ptr);
    ...
    if (Z_TYPE_P(prop) != IS_NULL) {
      ...
      compact_serialize(ttype, transport, prop, fieldspec);
    }
  }
  transport.writeI8(T_STOP); // struct end
}

// 6 params: $transport $method_name $ttype $request_struct $seqID
PHP_FUNCTION(thrift_protocol_write_compact) {
  ...
  if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS(), "oSlol",
    &protocol, &method_name, &msgtype,
    &request_struct, &seqID) == FAILURE) {
      return;
  }

  try {
    PHPOutputTransport transport(protocol);
    TCompactProtocol phpProtocol(&transport);
    protocol_writeMessageBegin(protocol, method_name, (int32_t) msgtype, (int32_t) seqID);
    phpProtocol.serialize(request_struct);
    transport.flush();
...
// 4 params: $transport $response_Typename $buffer_size
PHP_FUNCTION(thrift_protocol_read_compact_after_message_begin) {
  ...
  if (zend_parse_parameters(ZEND_NUM_ARGS(), "oS|l", &protocol, &obj_typename, &buffer_size) == FAILURE) {
    return;
  }
